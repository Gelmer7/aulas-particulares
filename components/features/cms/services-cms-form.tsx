...
export function ServicesCmsForm({ initialData }: { initialData: Record<string, any> | null }) {
  ...
  const toggleVisibility = (id: string) => {
      const updatedList = services.map(s => s.id === id ? { ...s, visible: !s.visible } : s);
      setServices(updatedList);
      saveToDatabase(updatedList);
  };
  ...
  return (
      <div className="flex flex-col gap-6 py-2">
          <Toast ref={toast} />
          <Message severity="info" text="Crie os cartões com os tipos de aulas ou pacotes que você oferece (ex: Revisão ENEM, Aulas Avulsas, Pacote Mensal)." />

          <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-bold text-slate-800">Serviços Oferecidos ({services.length})</h3>
              ...
              {editingService && (
                  <div className="flex flex-col gap-6 py-4">
                      <div className="flex flex-col gap-2">
                         <label className="font-semibold text-slate-700 text-sm">Imagem/Ícone (Opcional)</label>
                         <div className="w-40">
                            <ImageUploader 
                               currentImageUrl={editingService.image ?? undefined} 
                               folderId="services" 
                               onUploadSuccess={(url) => setEditingService({...editingService, image: url})} 
                            />
                         </div>
                      </div>

                      <div className="flex flex-col gap-2">
                         <label className="font-semibold text-slate-700 text-sm">Título *</label>
                         <InputText 
                      ...
                      <div className="flex flex-col gap-2">
                         <label className="font-semibold text-slate-700 text-sm">Resumo</label>
                         <InputTextarea 
                             rows={3} 
                      ...
                      <div className="grid grid-cols-2 gap-4">
                          <div className="flex flex-col gap-2">
                             <label className="font-semibold text-slate-700 text-sm">Preço (Opcional)</label>
                             <InputNumber 
                                 value={editingService.price} 
                                 onValueChange={(e) => setEditingService({...editingService, price: e.value || null})} 
                          ...
                          <div className="flex flex-col gap-2">
                             <label className="font-semibold text-slate-700 text-sm">Duração/Frequência</label>
                             <InputText 
